...
/**
 * Base OpenFDE System Server which handles the starting and states of various OpenFDE
 ...
 */
public class OpenfdeSystemServer {
    private static final String TAG = "OpenfdeSystemServer";
    private Context mSystemContext;
    private OpenfdeSystemServiceHelper mSystemServiceHelper;

    private static final String ENCRYPTING_STATE = "trigger_restart_min_framework";
    private static final String ENCRYPTED_STATE = "1";

    public OpenfdeSystemServer(Context systemContext) {
        mSystemContext = systemContext;
        mSystemServiceHelper = new OpenfdeSystemServiceHelper(mSystemContext);
    }
    ...
    private void run() {
        ...
        try {
            ...
            Slog.e("System", "******************************************");
            Slog.e("System", "************ Failure starting OpenFDE system services", ex);
            throw ex;
        }
    }
    ...
    private void startServices() {
        ...
        String[] externalServices = context.getResources().getStringArray(
                com.android.internal.R.array.config_externalOpenfdeServices);

        for (String service : externalServices) {
            try {
                Slog.i(TAG, "Attempting to start service " + service);
                OpenfdeSystemService fdeSystemService =  mSystemServiceHelper.getServiceFor(service);
                if (coreAppsOnly() && !fdeSystemService.isCoreService()) {
                    Slog.d(TAG, "Not starting " + service +
                            " - only parsing core apps");
                } else {
                    Slog.i(TAG, "Starting service " + service);
                    ssm.startService(fdeSystemService.getClass());
                }
            } catch (Throwable e) {
